...
Secondly, the mean lag $\bar{\tau}$ is not accessible
using the common
cross-correlation technique, except in the case of very narrow
transfer functions.
Direct physical modelling of the BLR resolves these
...
\begin{eqnarray}
\textnormal{CCF}(\tau) &=&
\sum_{i=1}^{N_c} \sum_{j=1}^{N_l} \left(\frac{(y_i - \bar{y})(l_j - \bar{l})}{\sigma_{y_i}^2\sigma_{l_j}^2}\right)
\delta\left[\tau - (t_{l_j} - t_{y_i})\right]
\end{eqnarray}
